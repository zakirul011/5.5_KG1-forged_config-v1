...
const invertBnt = document.querySelectorAll('.invert-btn')
...
const lastPreviewConfig = document.querySelectorAll('.last-preview-config img')
...
configTitle.addEventListener('click', ()=>{resetSeletedColor()})
seletedColor.forEach(color => {
   color.addEventListener('click', ()=>{resetSeletedColor()})
});
...
configTitle.addEventListener('click', ()=>{
   ...
   if (window.matchMedia("(max-width: 767px)").matches) {
      let scrollingheght = document.querySelector('.config-content').getBoundingClientRect().top;
      ...
      if (selectedColorArray.length < 2) {
         ...
         if (pageCounter == chooseColorPage.length - 1) {
            ...
            if (pageCounter < 0) {
               ...
               error.classList.add('active')
               errorText.innerText = "Please choose a color to go next step";
            }
            ...
            if (window.matchMedia("(max-width: 767px)").matches) {
               ...
               if (yy == 0) {
                  // window.scrollBy(0, document.querySelector('.config-content-wrap').getBoundingClientRect().top - 20);
                  if (window.matchMedia("(max-width: 767px)").matches) {
                     setTimeout(()=>{
                        let bottomNextBtn = document.querySelector('.config-wheel-container .next-btn')                   
                        let scrollingheght =  bottomNextBtn.getBoundingClientRect().top + bottomNextBtn.getBoundingClientRect().height * 2 - window.innerHeight + 20;
                        window.scrollBy(0, scrollingheght)
                     }, 200)
                  }
                  yy = 1
...
// GIVE SERICAL NUMBER TO NAVIGATION 
configNavigationLink.forEach((link, index) => {
   let sercialCarrier = link.querySelector('.confg-nav-serial')
   sercialCarrier.textContent = index + 1
});
...
configNavigationLink.forEach((link, index) => {
   link.addEventListener('click', ()=>{
      if (index == 0) {
         ...
         error.classList.add('active')
         errorText.innerText = "Please choose color a to go next pages";
      }
   ...
   color.addEventListener('click', ()=>{
      ...
      if (pageCounter == chooseColorPage.length - 1) {
         ...
         if (window.matchMedia("(max-width: 767px)").matches) {
            selectedBoxed.classList.add('active');
            if (yyy == 0) {
               window.scrollBy(0, selectedBoxed.clientHeight + 20)
               yyy = 1;
            }
         }
      }
...
function getImageSelect(color) {
   ...
   allIndexedImg = document.querySelectorAll(`.config-part-page:not(.config-fixed-page) .config-part-category:nth-child(${colorParentIndex + 1}) img:nth-child(${colorIndex + 1})`)
   ...
   color.addEventListener('click', ()=>{
      if (selectedColorArray.length >= 2) {
         if (color.classList.contains('active')) {
            ...
            error.classList.add('active')
            errorText.innerText = "You can choose only 2 colors";
         }
      }else{
         ...
         if (selectedColorArray.length > 1) {
            ...
            configTitle.querySelector('h6').textContent = 'Click here to change your wheel colors';

            if (window.matchMedia("(max-width: 767px)").matches) {
                setTimeout(()=>{
                   let bottomNextBtn = document.querySelector('.config-wheel-container .next-btn')                   
                  let scrollingheght =  bottomNextBtn.getBoundingClientRect().top + bottomNextBtn.getBoundingClientRect().height - window.innerHeight + 20;
                  window.scrollBy(0, scrollingheght)
               }, 200)
            }
         }
...
function finalWheelImg() {
   ...
   window.scrollTo(0,document.querySelector('.config-wrapper').getBoundingClientRect().top)
